...
"[project]/app/api/ies/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(req) {
    try {
        const raw = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$csv$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["fetchAba"])("ies");
        let dados = raw.map(__TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$types$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizaIES"]).filter((d)=>d.ies && d.ies.toLowerCase() !== "total");
        const { searchParams } = new URL(req.url);
        const edicao = searchParams.get("edicao");
        const uf = searchParams.get("uf");
        const modalidade = searchParams.get("modalidade");
        const busca = searchParams.get("busca")?.toLowerCase().trim();
        const ordenarPor = searchParams.get("ordenarPor") ?? "pctAprovadosPresentes";
        const ordem = searchParams.get("ordem") === "asc" ? "asc" : "desc";
        const pagina = Math.max(1, parseInt(searchParams.get("pagina") ?? "1", 10));
        const porPagina = Math.min(200, Math.max(1, parseInt(searchParams.get("porPagina") ?? "25", 10)));
        ...
        if (busca) dados = dados.filter((d)=>d.ies.toLowerCase().includes(busca) || d.sigla.toLowerCase().includes(busca) || d.cidade.toLowerCase().includes(busca));
        dados.sort((a, b)=>{
        ...
        const ufsDisponiveis = Array.from(new Set(raw.map((r)=>(r["UF"] ?? "").trim()).filter(Boolean))).sort();
        const modalidadesDisponiveis = Array.from(new Set(raw.map((r)=>(r["Modalidade"] ?? "").trim()).filter(Boolean))).sort();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0nji87v._.js.map